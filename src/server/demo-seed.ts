/**
 * Base de demonstração do CSCX — Escola Instructiva.
 *
 ...
 *
 * Usada pelo script `npm run db:seed` e pelo botão "Popular com dados de
 * demonstração" do painel, quando a base ainda está vazia.
 */
...
import { ONBOARDING_CHECKLIST, INTEGRATION_CATALOG } from '@/lib/constants';
import { classifyNps } from './survey-service';
import { ensureDefaultAutomations } from './automation-service';
import { runDailyRoutine } from './routine';
import { DEFAULT_THRESHOLDS, DEFAULT_WEIGHTS } from '@/lib/health-score';
import { healthScoreConfig } from '@/db/schema';
...
/**
 * Limpa a base antes de popular.
 * Com `keepUsers`, a tabela de usuários é preservada — assim quem está logado
 * continua logado e mantém as próprias credenciais.
 */
async function reset(keepUsers: boolean) {
  console.log(`→ limpando base${keepUsers ? ' (mantendo os usuários)' : ''}…`);
...
export async function seedDemoData(
  opts: { reset?: boolean; keepUsers?: boolean } = {},
): Promise<SeedResult> {
  const RESET = opts.reset ?? false;
  const KEEP_USERS = opts.keepUsers ?? false;
  console.log('CSCX — populando a base de demonstração');

  if (RESET) {
    await reset(KEEP_USERS);
  } else {
    const jaTemAlunos = await db.select({ id: students.id }).from(students).limit(1);
    if (jaTemAlunos.length) {
      throw new Error(
        'A base já tem alunos. Use a opção de recriar para apagar tudo e popular de novo.',
      );
    }
  }
  ...
  // Reaproveita quem já existe (mesmo e-mail) e cria só o que falta, para não
  // derrubar a conta de quem está usando o sistema agora.
  const existentes = await db.select().from(users);
  const porEmail = new Map(existentes.map((u) => [u.email.toLowerCase(), u]));
  const faltando = TEAM.filter((t) => !porEmail.has(t.email.toLowerCase()));

  const novos = faltando.length
    ? await db
        .insert(users)
        .values(faltando.map((t) => ({ ...t, passwordHash })))
        .returning()
    : [];

  const team = TEAM.map(
    (t) =>
      porEmail.get(t.email.toLowerCase()) ??
      novos.find((n) => n.email.toLowerCase() === t.email.toLowerCase())!,
  );
  console.log(`✓ ${team.length} usuários da equipe (${novos.length} criados agora)`);
  ...
  const jaExiste = porEmail.get('aluno@exemplo.com.br');
  const [studentUser] = jaExiste
    ? [jaExiste]
    : await db
        .insert(users)
        .values({
  ...
  console.log('✅ Base de demonstração pronta.');

  return {
    usuarios: team.length + 1,
    cursos: courseRows.length,
    alunos: createdStudents.length,
    senha: process.env.SEED_PASSWORD ?? 'cscx2026',
    adminEmail: 'admin@escolainstructiva.com.br',
  };
}
