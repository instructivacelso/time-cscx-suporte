...
/**
 * Scripts de mensagem da equipe: os textos prontos usados no WhatsApp, no
 * e-mail e nas ligações. Ficam no banco (e não no código) para a equipe
 * escrever e ajustar sozinha.
 */
export const messageScripts = pgTable(
  'message_scripts',
  {
    id: id(),
    title: text('title').notNull(),
    channel: text('channel').default('WHATSAPP').notNull(),
    situation: text('situation').default('GERAL').notNull(),
  ...
  (t) => ({
    channelIdx: index('scripts_channel_idx').on(t.channel),
    situationIdx: index('scripts_situation_idx').on(t.situation),
  }),
);
