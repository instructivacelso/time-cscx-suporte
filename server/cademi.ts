 ...
 */
/**
 * O número pode vir direto (`"progress": 50`) ou dentro de um objeto
 * (`"progress": { "percentage": 50 }`). Aqui aceitamos as duas formas.
 */
function numeroDe(valor: unknown): number | null {
  if (valor === undefined || valor === null) return null;
  if (!isObj(valor)) return numero(valor);

  const interno = buscar(valor, [
    'percentage',
    'percentual',
    'porcentagem',
    'percent',
  ...
  return interno !== undefined && !isObj(interno) ? numero(interno) : null;
}
...
export function lerPayload(body: unknown): CademiPayload {
  ...
  return {
    ...
    nota: numeroDe(buscar(body, ['nota', 'score', 'pontuacao', 'grade', 'acertos'])),
  };
}
